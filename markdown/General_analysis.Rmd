...
df_all_ST_filtered_SNPs_all %>%
    ...
    mutate(value = as.factor(value)) %>%
    rename('Allelic\nDistance\n(cgMLST)' = value) %>%
    group_by(source_combo, ST.x) %>% tally() %>% rename("Source Combination" = source_combo, "Sequence Type" = ST.x, "Count of Strain Pairs" = n) %>% write_delim("../figures/Final_Figures/Supplementary_Material/Supplementary_Table_3.tsv", delim = "\t")
...
UZ_clusters <- read_delim("../../APG-OHEC-Retro-M1/Revision/APG_manuscript_1_UZ.clusters.tsv",
                                 delim = "\t", escape_double = FALSE,
                                 trim_ws = TRUE)

UZ_clusters <- UZ_clusters %>% mutate(ID = str_replace(ID, ".result", "")) 

# If "../../APG-OHEC-Retro-M1/Revision/APG_manuscript_1_UZ.distances.tsv.gz" is present and "../../APG-OHEC-Retro-M1/Revision/APG_manuscript_1_UZ.distances.tsv" isnt, unzip the file
if (file.exists("../../APG-OHEC-Retro-M1/Revision/APG_manuscript_1_UZ.distances.tsv.gz") & !file.exists("../../APG-OHEC-Retro-M1/Revision/APG_manuscript_1_UZ.distances.tsv")) {
  system("gunzip ../../APG-OHEC-Retro-M1/Revision/APG_manuscript_1_UZ.distances.tsv.gz")
}

UZ_dists <- read_delim("../../APG-OHEC-Retro-M1/Revision/APG_manuscript_1_UZ.distances.tsv",
                                 delim = "\t", escape_double = FALSE,
                                 trim_ws = TRUE)
