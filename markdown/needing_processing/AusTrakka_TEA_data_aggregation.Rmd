...
```{r}
#Read in our tree
tree <- ggtree::read.tree("APG-OHEC-Retro-M1/analysis/RapidNJ_5471.nwk")
...
### TEA genome data
TEA_abritamr_path <- "APG-OHEC-Retro-M1/analysis/abritamr_resistance.txt"

TEA_mlst_path <- "APG-OHEC-Retro-M1/analysis/mlst.txt"

TEA_clermontyping_path <- "APG-OHEC-Retro-M1/analysis/clermontyping.txt"

TEA_ectyper_path <- "APG-OHEC-Retro-M1/analysis/ectyper.txt"

TEA_fimtyper_path <- "APG-OHEC-Retro-M1/analysis/fimtyper.txt"

TEA_IncF_RST_path <- "APG-OHEC-Retro-M1/analysis/IncF_RST.txt"

genotype_path_TEA <- "APG-OHEC-Retro-M1/analysis/genotype.txt"
```
...
if(!file.exists(genotype_path_TEA)){
  unzip(exdir = "APG-OHEC-Retro-M1/analysis", zipfile = "APG-OHEC-Retro-M1/analysis/genotype.txt.zip", overwrite = FALSE)
}
...
#Read in QC data
QC_pass_names <- read_csv("APG-OHEC-Retro-M1/delims/QC_pass_names.txt", show_col_types = FALSE)

#Remove samples which failed QC
geno <- geno %>% filter(name %in% QC_pass_names$name)

#Read in the names of the quarantined samples
WW_quarantine <- read_csv("APG-OHEC-Retro-M1/delims/purgelist.txt", col_names = 'name', show_col_types = FALSE)

#Remove WW samples
geno <- geno %>% filter(name %nin% WW_quarantine$name)

#Generate list of potential gull quarantined samples (2019 cohort unpublished)
Gull_quarantine <- read_csv("APG-OHEC-Retro-M1/delims/purgelist2.txt", col_names = 'name', show_col_types = FALSE)
...
```{r file_paths, echo=FALSE}
#Read in QC data
metadata <- read_delim("APG-OHEC-Retro-M1/delims/metadata.csv", 
                   delim = ",",
                   escape_double = FALSE,
                   trim_ws = TRUE)
...
convert_tea <- read_csv("APG-OHEC-Retro-M1/delims/name_convert_TEA.txt", show_col_types = FALSE)
...
abritamr <- read_delim("APG-OHEC-Retro-M1/analysis/abritamr_resistance.txt", 
...
#Write to file
long_format_abritamr_processed%>% write_delim("APG-OHEC-Retro-M1/delims/long_format_abritamr_5471.txt", delim = "\t")
...
write_delim(genometa, "APG-OHEC-Retro-M1/delims/genometa_n5471.txt", delim = "\t")
```
