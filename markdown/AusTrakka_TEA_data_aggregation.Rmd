...
# If "../analysis/genotype.txt.gz" is present and "../analysis/genotype.txt" isnt, unzip the file
if (file.exists("../analysis/genotype.txt.gz") & !file.exists(".../analysis/genotype.txt")) {
  system("gunzip ../analysis/genotype.txt.gz")
}
...
#Remove genomes which are not in our tree
geno <- filter(geno, name %in% tree$tip.label)
...
```{r file_paths, echo=FALSE, message=FALSE, warning=FALSE}
#Read in metadata
...
#Replace instances of "Other Mammal" with "Mammal" in revised source type and source type
metadata <- metadata %>% mutate(Revised_Source_Type = if_else(Revised_Source_Type == "Other Mammal", "Mammal", Revised_Source_Type)) %>% mutate(Source_Type = if_else(Source_Type == "Other Mammal", "Mammal", Source_Type))
...
```{r ST_exploration, echo=FALSE, message=FALSE, warning=FALSE}
...
#Write to file
long_format_abritamr_processed%>% write_delim("../delims/long_format_abritamr_5471.txt", delim = "\t")
...
write_delim(genometa, "../delims/genometa_n5471.txt", delim = "\t")
```
