...
for(sequence_type in unique(distances2$ST)){
  ...
  metadata <- genometa %>%
    ...
    select(name, Revised_Source_Niche, Collection_Year, State, Collection, fimtyper..Fimtype, ectyper..Serotype, `IncF_RST..F type`, ColV)
  ...
  ColV <- sort(unique(metadata$ColV))
  
  library(hues)
  
  ColV_cols <- iwanthue(length(ColV), plot = FALSE)
  
  names(ColV_cols) <- ColV
  
  #Get a list of vertex names
  g_vertex_names <- V(g)$name 
  
  # Match colors to vertices based on names
  vertex_color_vector_ColV <- metadata %>% mutate(colours = ColV_cols[as.character(ColV)]) %>% pull(colours)
  
  
  # Plot the graph
  pdf(file = paste0("../figures/Network_graphs/Network_by_ColV_",
                    sequence_type,
                    ".pdf"),
      width = 11.69,
      height = 8.27)
  ...
  plot(g, edge.width = E(g)$thickness,
       ...
       vertex.size = 3,
       vertex.label = NA,
       vertex.color = vertex_color_vector_ColV,
  )
  
  legend("bottomleft",
         legend = names(ColV_cols),
         fill = unname(ColV_cols),
         pch=19,
         bty = "n",
         pt.cex = 1,
         cex = 1,
