...
for(sequence_type in unique(distances2$ST)){
  ...
  metadata <- genometa %>%
    ...
    select(name, Revised_Source_Niche, Collection_Year, State, Collection, fimtyper..Fimtype, ectyper..Serotype)
  ...
  serotypes <- sort(unique(metadata$ectyper..Serotype))
  
  library(hues)
  
  serotypes_cols <- iwanthue(length(serotypes), plot = FALSE)
  
  names(serotypes_cols) <- serotypes
  
  #Get a list of vertex names
  g_vertex_names <- V(g)$name 
  
  # Match colors to vertices based on names
  vertex_color_vector_serotype <- metadata %>% mutate(colours = serotypes_cols[as.character(ectyper..Serotype)]) %>% pull(colours)
  
  
  # Plot the graph
  pdf(file = paste0("../figures/Network_graphs/Network_by_serotype_",
                    sequence_type,
                    ".pdf"),
      width = 11.69,
      height = 8.27)
  ...
  plot(g, edge.width = E(g)$thickness,
       ...
       vertex.size = 3,
       vertex.label = NA,
       vertex.color = vertex_color_vector_serotype,
  )
  
  legend("bottomleft",
         legend = names(serotypes_cols),
         fill = unname(serotypes_cols),
         pch=19,
         bty = "n",
         pt.cex = 1,
         cex = 1,
