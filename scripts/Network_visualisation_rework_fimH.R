...
for(sequence_type in unique(distances2$ST)){
  ...
  fimtypes <- sort(unique(metadata$fimtyper..Fimtype))
  
  library(hues)
  
  fimtype_cols <- iwanthue(length(fimtypes), plot = FALSE)
  
  names(fimtype_cols) <- fimtypes
  
  #Get a list of vertex names
  g_vertex_names <- V(g)$name 
  
  # Match colors to vertices based on names
  vertex_color_vector_fimH <- metadata %>% mutate(colours = fimtype_cols[as.character(fimtyper..Fimtype)]) %>% pull(colours)
  
  
  # Plot the graph
  pdf(file = paste0("../figures/Network_graphs/Network_by_fimH_",
                    sequence_type,
                    ".pdf"),
      width = 11.69,
      height = 8.27)
  ...
  plot(g, edge.width = E(g)$thickness,
       ...
       vertex.size = 3,
       vertex.label = NA,
       vertex.color = vertex_color_vector_fimH,
       )
  
  legend("bottomleft",
         legend = names(fimtype_cols),
         fill = unname(fimtype_cols),
         pch=19,
         bty = "n",
         pt.cex = 1,
         cex = 1,
